...
Namespace Models
    Public NotInheritable Class ChartViewModel
        ...
        '====================================================================================================
        '
        Public Shared Function getViewingChart(ae As Controllers.applicationController, viewingSummaryList As List(Of Models.viewingSummaryModel), Div As String, isVisitData As Boolean, Optional Width As String = "100%", Optional Height As String = "400px", Optional AllowHourly As Boolean = False) As String
            Dim result As New StringBuilder
            Try
                If Width = "" Then
                    Width = "100%"
                ...
                result.Append("data.addColumn('date', 'Date');" & vbCrLf)
                result.Append("data.addColumn('number', '" & Caption & "');" & vbCrLf)
                result.Append("data.addRows(" & viewingSummaryList.Count & ");" & vbCrLf)
                '
                Dim Pointer As Integer = 0
                For Each viewSummary As Contensive.Addons.Reporting.Models.viewingSummaryModel In viewingSummaryList
                    Dim Value As String = String.Empty
                    Dim nrmDate As Date
                    If (viewSummary.TimeNumber <> 0) And (AllowHourly) Then
                        nrmDate = Date.FromOADate((viewSummary.DateNumber + (viewSummary.TimeNumber / 24.0!)))
                    ...
                    Value = viewSummary.PageViews.ToString()
